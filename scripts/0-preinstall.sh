...
# open luks container and ROOT will be place holder 
    echo -n "${LUKS_PASSWORD}" | cryptsetup open ${partition3} cryptpart -
# now format that container
    mkfs.btrfs -L cryptpart ${partition3}
...
cp -R ${SCRIPT_DIR} /mnt/root/arch-sway
...
if [[  $TOTAL_MEM -lt 16000000 ]]; then
    ...
    mkdir -p /mnt/swap # make a dir that we can apply NOCOW to to make it btrfs-friendly.
    chattr +C /mnt/swap # apply NOCOW, btrfs needs that.
    dd if=/dev/zero of=/mnt/swap/swapfile bs=1M count=8000 status=progress
